...
| expand | session turn [--before N] [--after N] [--only classifier] | Classifier lines (index, time, role, type, chars) around an anchor turn; before/after default 30 with a HARD FLOOR of 30 — only larger allowed; `--only` narrows what is printed, never the examined window |
| expand --full | session turn --full --before N --after N [--only classifier] | Full content of the window; both bounds required, no floor (0 works) |
...
`--only` takes a role (`user`), a type (`tool_result`), or a role/type pair (`user/text`) — the pair isolates what the human actually typed.
